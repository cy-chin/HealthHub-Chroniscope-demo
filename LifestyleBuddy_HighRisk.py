...
with tab1:
    ...
    with st.form("questionnaire_form"):
        ...
        x_age_AGEG5YR = get_age_group(left.number_input("Age", min_value = 18, max_value=99, value=35))
        x_gender_SEX = gender_dict[right.selectbox("Gender",gender_dict.keys(), index=0)]
        x_weight_WTKG3 = left.number_input("Weight (in KG)", min_value = 30, max_value = 150, value=90 )
        x_height_HTM4 = right.number_input("Height (in CM)", min_value = 100, max_value = 220, value=160)
        x_marital_MARITAL = marital_dict[left.selectbox("Marital Status", marital_dict.keys(), index = 0)]
        x_edu_EDUCAG = edu_dict[right.selectbox("Highest Education Level", edu_dict.keys(), index = 4 )]
        x_employment_EMPLOY1 = employment_dict[left.selectbox("Employment Status", employment_dict.keys())]
        x_blind_BLIND = yes_no_dict[ right.selectbox("Difficulty seeing, even when wearing glasses?", yes_no_dict.keys(), index=1)]
        ...
        x_alch_binge_RFBING5 = yes_no_dict[ right.selectbox("Binge drinker ( >4 drinks on one occasion)?", yes_no_dict.keys(), index=1)]
